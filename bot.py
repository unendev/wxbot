...
class ChatSessionState:
    ...
    def send_text_reply(self, reply_text: str) -> bool:
        ...
        try:
            ...
            # 真实人类反应：阅读理解/构思时延 (1.8s ~ 3.2s) + 拼音打字速率 (约 0.055s ~ 0.075s/字，平滑上限 6.5s)
            base_jitter = random.uniform(1.8, 3.2)  # 随机阅读理解与操作思考抖动
            typing_rate = random.uniform(0.055, 0.075)  # 拟人拼音打字速率 (每字约 55-75ms)
            typing_delay = min(len(reply_text) * typing_rate, 6.5)  # 动态打字耗时，平滑上限 6.5s
            total_delay = base_jitter + typing_delay
            logger.info("[%s] Cadence delay injected: %.2fs (read/jitter: %.2fs, typing: %.2fs for %d chars)", 
                        self.name, total_delay, base_jitter, typing_delay, len(reply_text))
